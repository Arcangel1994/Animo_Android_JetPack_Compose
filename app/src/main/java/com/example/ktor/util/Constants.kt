package com.example.ktor.util

object Constants {

    const val BASE_URL = "http://10.0.2.2:8080"
    const val BASE_URL_HEROKU = "http://boruto-server.herokuapp.com"
    const val BASE_URL_HEROKU_SECURITY = "https://boruto-server.herokuapp.com"

    const val DETAILS_ARGUMENT_KEY = "heroId"

    const val BORUTO_DATABASE = "boruto_database"

    const val HERO_DATABASE_TABLE = "hero_table"
    const val HERO_REMOTE_KEYS_DATABASE_TABLE = "hero_remote_keys_table"

    const val ON_BOARDING_PAGE_COUNT = 3

    const val LIST_ON_BOARDING_PAGE = 2

    const val PREFERENCES_NAME = "boruto_preferences"
    const val PREFERENCES_KEY = "on_boarding_completed"

    const val ITEMS_PER_PAGE = 3
    const val ABOUT_TEXT_MAX_LINE = 7

    const val MIN_BACKGROUND_IMAGE_HEIGHT = 0.5f

    const val VIBRANT = "vibrant"
    const val DARKVIDRANT = "darkVibrant"
    const val ONDARKVIBRANT = "onDarkVibrant"

}